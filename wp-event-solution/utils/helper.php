<?php
                ...
                <div class="search-button-wrapper">
					...
					<?php if ( defined( 'ETN_PRO_FILES_LOADED' ) ) : ?>
                        <button
                                ...
                                aria-label="<?php echo esc_attr__( 'Event filter button', 'eventin' ); ?>"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                                 fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
					...
					<div class="etn-col-md-6 etn-col-lg-<?php echo esc_attr(apply_filters('etn_event_archive_column', '4')); ?>">
						<div class="etn-event-item">
							...
							<div class="etn-event-content">
								...
								<p><?php echo wp_kses_post( apply_filters( 'etn_event_archive_content', wp_trim_words( $excerpt, 15 , '' ) ) ); ?></p>
                            </div>
	...
	<?php
		...
		<?php if ( ('online' === $event_type || 'hybrid' === $event_type) && $meeting_link ): ?>
			<div><?php echo esc_html( $platforms[$platform] ); ?> :  <a href="<?php echo esc_url( $meeting_link ); ?>" target="_blank" class="" style="margin-right: 10px"><?php echo esc_url( $meeting_link ); ?></a></div>
		<?php endif; ?>
		...
		?>
        <div>
			<?php echo wp_kses_post( sprintf( $purchase_email_message, esc_html( $event_name ) ) ); ?>
        </div>
        <br><br>
		<?php
     ...
     */
                    ...
                    public static function get_ticket_variations_info($event_id = null, $ticket_variations = [], $from_multivendor = false)
                    {
                        ...
                        foreach ($ticket_variations as $key => &$value) {
                            // will optimize
                            if (
                                ! empty($value['etn_ticket_name']) || ! empty($value['etn_ticket_price']) || ! empty($value['etn_avaiilable_tickets']) ||
                                ...
                                $is_unlimited = ! empty( $value['etn_unlimited_tickets'] ) || ( isset( $value['etn_avaiilable_tickets'] ) && intval( $value['etn_avaiilable_tickets'] ) === -1 );
                                $etn_avaiilable_tickets          = $is_unlimited ? -1 : ( ! empty($value['etn_avaiilable_tickets']) ? intval($value['etn_avaiilable_tickets']) : 100000 );
                                ...
                                $etn_max_ticket = ! empty($value['etn_max_ticket']) ? intval($value['etn_max_ticket']) : ( $is_unlimited ? 0 : $etn_avaiilable_tickets );

                                if ( ! $is_unlimited && $etn_min_ticket > $etn_avaiilable_tickets) {
                                    $etn_min_ticket = 1;
                                }

                                if ( ! $is_unlimited && $etn_max_ticket > $etn_avaiilable_tickets) {
		...
		<?php
                 ...
                 * @return string
                 */
                public static function filter_template_tags($content, $event_name)
                {
