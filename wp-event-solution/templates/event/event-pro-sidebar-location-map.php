...
$locations_html = esc_html__( 'No event location is added yet.', 'eventin' );
...
if ( !empty( $all_locations ) ) {
    ...
    foreach ( $all_locations as $key => $location ) {
        ...
        $location_longitude = get_term_meta( $term_id, 'location_longitude', true );
            ...
            $loc_image = \Wpeventin::assets_url() . 'images/placeholder.jpg';
            if ( ! empty( $image_id ) ) {
        ...
        $msg = esc_html__( 'No store found in this location', 'eventin');
}
...
if ( !empty( $locations_html_data ) ) {
        ...
        // Defense-in-depth: never load the Google Maps API when the addon is disabled.
        $etn_maps_enabled = ( 'on' === etn_get_option( 'etn_googlemap_api' ) ) && etn_get_option( 'google_api_key' );

        if ( !empty( $all_locations ) && $etn_maps_enabled ) {
