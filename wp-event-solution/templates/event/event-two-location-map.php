...
<?php if ( 
    ...
    $google_map_api_key ): ?>
<?php
// The interactive Google Map embed relies on the Pro maps subsystem
// (etn-map/etn-location scripts + their AJAX endpoint). Load it only when Pro is
// active; the venue heading, address and "View on Map" link below render with the
// free plugin alone, so layouts two & three still show location info without Pro.
$etn_interactive_map = class_exists( 'Wpeventin_Pro' );
if ( $etn_interactive_map ) {
    // PERF: point-of-use enqueue — Maps API loads only when this map actually renders.
    wp_enqueue_script( 'etn-map' );
    wp_script_add_data( 'etn-map', 'async', true );
    wp_enqueue_script( 'etn-location' );
}
?>
<h3 class="etn-locations-title">
    <?php
    $location_title = apply_filters( 'etn_event_location_title', esc_html__('Venue Info', 'eventin') );
...
<div class='etn-single-location-item  etn-location-item'>
    <div class="etn-single-location-item-content">
        ...
        <a href="http://maps.google.com/maps?q=<?php echo esc_attr($latitudes); ?>,<?php echo esc_attr($longitudes); ?>" class="etn-btn etn-primary view-map-button" target="_blank"><?php echo esc_html__('View on Map', 'eventin'); ?></a>
    </div>
</div>
